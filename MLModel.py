...
class MLModel:
    ...
    def preprocessing_pipeline(self, df):
        ...
        # Create features
        df = MLModel.create_new_features(df, "V25")
        df = MLModel.create_new_features(df, "V26")
        df = MLModel.create_new_features(df, "V27")
    ...
    def preprocessing_pipeline_inference(self, sample_data):
        ...
        sample_data = self.create_new_features(sample_data, "V25")
        sample_data = self.create_new_features(sample_data, "V26")
        sample_data = self.create_new_features(sample_data, "V27")
    ...
    def create_new_features(df, col_name):
        """
        Create new features based on the description provided.
        ...
        df[modified_col_name] = df[col_name].apply(lambda x: str(x) 
                                    if pd.notna(x) and len(str(x)) > 3 else None)

        df[f"{col_name}_lesion_site"] = df[modified_col_name].apply(
            lambda x: int(x[:2]) if x and x[:2] in ['00', '11'] 
            else int(x[0]) if x else None
        )

        df[f"{col_name}_type"] = df[modified_col_name].apply(
            lambda x: int(x[2]) if x and x[:2] in ['00', '11'] 
            else int(x[1]) if x else None
        )

        df[f"{col_name}_subtype"] = df[modified_col_name].apply(
            lambda x: int(x[3]) if x and x[:2] in ['00', '11'] 
            else int(x[2]) if x else None
        )

        df[f"{col_name}_specific_code"] = df[modified_col_name].apply(
            lambda x: int(x[4:]) if x and len(x) > 4 and x[3:] != '10' 
            else int(x[3:]) if x and len(x) > 3 else None
        )
